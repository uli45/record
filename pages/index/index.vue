<template>
	<view>
		...
		<view class="recBox">
			...
			<u-popup :show="show" @close="close" @open="show = true" :round="10" closeable :closeOnClickOverlay='false'>
				<view class="title">
					今天有奖励自己吗？
				</view>
				<view class="popBox">
					<u--form labelPosition="left" :model="model" :rules="rules" ref="uForm">
						<u-form-item label="日期" prop="date" borderBottom ref="item1">
							<u--input v-model="model.date" border="none" readonly></u--input>
						</u-form-item>
						<u-form-item required labelWidth="120" label="是否有奖励自己" prop="type" borderBottom ref="item1">
							<u-radio-group v-model="model.type">
								<u-radio :customStyle="{marginRight: '8px'}" label='是' :name='2'></u-radio><u-radio
									label='否' :name='1'></u-radio>
							</u-radio-group>
						</u-form-item>
						<u-form-item required labelWidth="60" :label="model.type ===1 ?'心得' :'忏悔词'" prop="text"
							borderBottom ref="item1">
							<u--input v-model="model.text" border="none" maxlength='10'></u--input>
						</u-form-item>
		...
		<u-modal @cancel='cancel' :show="showModal" :title="title" @confirm="confirm" showCancelButton>
	...
	export default {
		data() {
			return {
				...
				days: uni.getStorageSync('days') || 0,
				notice: 'veshi',
				contentList: [
					'欲戒色，有次第，若依此，少费力。',
		...
		methods: {
			...
			close() {
				...
				// console.log('close');
				this.content =
					`您已经坚持打卡记录<span style='color:#18bc37;font-weight: 700;'>戒撸${uni.getStorageSync('days')||0}天</span>，真的要放弃打卡吗?`
			},
			confirm() {
			...
			cancel() {
				this.showModal = false
				uni.$u.toast(`兄弟加油，你已经坚持了${uni.getStorageSync('days')||0 }天`)
			},
			submit() {
				const length = Math.round(Math.random() * jsData.length)
