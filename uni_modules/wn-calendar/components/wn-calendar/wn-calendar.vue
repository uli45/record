<template>
	<view class="wn-calendar">
		...
		<view :class="['row', isBorder ? 'border' : '']" v-for="row in rows" :key="row">
			<view @click="onChoose(item)" :class="[
				'day-item', 
				item.show ? 'show' : '', 
				active === item.date ? 'active' : ''
			]" v-for="(item, ind) in days.slice(row*7, (row+1)*7)" :key="item.id">
				<text class="t">{{item.label}}</text>
				<text v-if="item.data && item.data.text" class="data"
					:style="{ color:`${colors[item.data.type] || '#f3a73f'}` }">{{item.data.text}}</text>
			</view>
			...
			refresh() {
				this.$nextTick(() => {
					const {
						...
						year,
						month
					} = calendar.getDays(this.year, this.month, this.data, this.isLess)
					this.days = days
					this.year = year
					this.month = month + 1
				})
